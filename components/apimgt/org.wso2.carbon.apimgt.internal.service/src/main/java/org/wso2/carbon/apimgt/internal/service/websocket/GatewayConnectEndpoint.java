 ...
 * against the control plane database. On success, the connection is associated with the gateway
 * and the gateway's active status is set to true; on close it is set to false. On verification
 * failure, the connection is closed with code 4401 (Unauthorized).
 */
@ServerEndpoint(
...
public class GatewayConnectEndpoint {
    ...
    public void onOpen(Session session, EndpointConfig config) {
        ...
        if (gateway == null) {
            if (log.isDebugEnabled()) {
                log.debug("Gateway WebSocket connection rejected: token did not match any registered gateway");
            }
            closeWithUnauthorized(session, "Invalid or expired API key");
            return;
        }
